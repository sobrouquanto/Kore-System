...
export default function ConfiguracoesTab() {
  const { user, userName, userBiz, saveConfig, doLogout } = useDashboard()
  ...
  async function handleChangePwd() {
    ...
    else { setPwdMsg('Senha alterada com sucesso! ✓'); setPwdOld(''); setPwdNew('') }
    ...
    <div style={{ maxWidth: '680px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '12px' }}>
          <div className="form-group-sm" style={{ flex: 1 }}>
            ...
            <input
              value={cfgCnpj}
              onChange={e => setCfgCnpj(formatCnpj(e.target.value))}
        ...
        {cnpjError && (
          <div style={{ background: 'var(--red-dim)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '10px', padding: '12px', fontSize: '13px', color: 'var(--red)' }}>
            ⚠️ {cnpjError}
          </div>
        )}
        {cnpjData && (
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <button
            className="btn-add"
            style={{ alignSelf: 'flex-start', minWidth: '160px', background: 'rgba(139,92,246,0.15)', border: '1px solid rgba(139,92,246,0.3)', color: '#a78bfa' }}
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {[
            ['E-mail', user?.email],
            ['Plano', 'MEI 360 OS · Plano Ativo'],
            ['Usuário desde', user?.created_at ? new Date(user.created_at).toLocaleDateString('pt-BR') : '—'],
          ].map(([label, value], i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: i < 2 ? '1px solid var(--card-border)' : 'none' }}>
      ...
      <div className="card" style={{ border: '1px solid rgba(239,68,68,0.2)', background: 'rgba(239,68,68,0.04)' }}>
        ...
        <button
          onClick={doLogout}
          style={{ background: 'var(--red-dim)', border: '1px solid rgba(239,68,68,0.3)', color: 'var(--red)', padding: '10px 20px', borderRadius: '10px', fontSize: '13px', fontWeight: 700, cursor: 'pointer' }}
        >
          Sair da conta →
        </button>
      </div>
